...
describe('tablePreferences', () => {
  ...
  it('uses configured defaults when app config is valid', () => {
    ...
    expect(getConfiguredTablePageSizeOptions()).toEqual([20, 50, 100, 500, 1000, 2000, 5000, 10000])
  })
  ...
  it('normalizes invalid options without rewriting the configured default itself', () => {
    window.__APP_CONFIG__ = {
      ...
      table_page_size_options: [10001, 50, 10, 10, 2, 0]
    } as any

    expect(getConfiguredTableDefaultPageSize()).toBe(35)
    expect(getConfiguredTablePageSizeOptions()).toEqual([10, 50, 500, 1000, 2000, 5000, 10000])
  ...
  it('normalizes page size against configured options by rounding up', () => {
    ...
    expect(normalizeTablePageSize(100)).toBe(500)
    expect(normalizeTablePageSize(1500)).toBe(2000)
    expect(normalizeTablePageSize(undefined)).toBe(20)
  })

  it('keeps built-in selectable defaults including the large page size options', () => {
    window.__APP_CONFIG__ = {
      table_default_page_size: 1000
    } as any

    expect(getConfiguredTablePageSizeOptions()).toEqual([10, 20, 50, 100, 500, 1000, 2000, 5000, 10000])
  })
})
