  ...
  admin: {
    ...
    groups: {
      ...
      accountModelRestrictions: {
        action: '模型限制',
        title: '批量设置账号模型限制',
        impactScope: '修改的是账号本体配置；这些账号在其他分组中被使用时也会同步受影响。',
        targetCount: '当前分组共有 {count} 个账号',
        openaiPassthroughHint: '如账号已开启 OpenAI 自动透传，运行时仍可能忽略这里设置的模型限制。',
        emptyWhitelistHint: '白名单留空时将清空现有限制，恢复为支持所有模型。',
        mappingHint: '映射模式会直接写入账号 credentials.model_mapping，行为与账号批量编辑一致。',
        submit: '应用到分组内账号',
        submitting: '应用中...',
        updated: '已更新 {success} 个账号，失败 {failed} 个',
        failed: '批量设置账号模型限制失败'
      },
    ...
    accounts: {
      ...
      dataExportGroupHint: '未选择分组时将导出全部账号。',
      dataImport: '导入',
      ...
      dataImportWarning: '导入将创建新账号与代理，账号会归入所选分组；请确认已有数据不会冲突。',
      dataImportTargetGroup: '目标分组',
      dataImportTargetGroupRequired: '必填',
      dataImportTargetGroupPlaceholder: '请选择目标分组',
      dataImportTargetGroupHint: '导入的账号会自动归入该分组。',
      dataImportTargetGroupMissing: '请选择目标分组',
      dataImportFile: '数据文件',
      ...
      allRefreshStates: '全部刷新状态',
      refreshStateSet: '已刷新',
      refreshStateUnset: '未刷新',
      allTestStates: '全部测试状态',
      testStateSet: '已测试',
      testStateUnset: '未测试',
      privacyUnset: '未设置',
      ...
      groupTransfer: {
        title: '账号分组迁移',
        description: '按数量将可迁移账号从源分组移动到目标分组。仅允许迁移同类型、状态正常、未限流且未在使用中的账号。',
        sourceGroup: '源分组',
        targetGroup: '目标分组',
        count: '迁移数量',
        selectGroups: '请选择源分组和目标分组',
        groupsMustDiffer: '源分组和目标分组不能相同',
        invalidCount: '请输入大于 0 的迁移数量',
        submit: '开始迁移',
        transferring: '迁移中...',
        success: '成功迁移 {count} 个账号',
        failed: '分组迁移失败'
      },
    ...
    scheduledTests: {
      ...
      groupDialogTitle: '分组定时测试',
      group: '分组',
      selectGroup: '选择分组',
      selectGroupFirst: '请先选择分组',
      createGroupPlan: '创建分组计划',
      planList: '计划列表',
      batchSize: '批次大小',
      offsetSeconds: '偏移秒数',
      modelOptional: '模型（可选）',
      modelOptionalHint: '留空则使用默认测试模型',
      cronRequired: '请输入 Cron 表达式',
      batchSizeInvalid: '批次大小必须大于 0',
      offsetInvalid: '偏移秒数必须大于等于 0',
      planCreated: '分组计划创建成功',
      planUpdated: '分组计划更新成功',
      planDeleted: '分组计划已删除',
      batchOffsetSummary: '批次 {batch} / 偏移 {offset}s',
      runNow: '测试账号',
      runNowConfirm: '确认立即测试分组“{name}”下的账号吗？',
      realtimeLogs: '实时日志',
      realtimeLogsTitle: '实时日志 - {name}',
      realtimeLogsHint: '选择最近任务查看实时进度与账号结果',
      recentJobs: '最近任务',
      noJobs: '暂无测试任务',
      selectJobFirst: '请选择一个任务查看详情',
      jobStatus: '任务状态',
      pending: '待执行',
      succeeded: '成功',
      jobLogs: '任务日志',
      ...
      scheduledFor: '计划时间',
      latencyMs: '耗时(ms)',
      progressSummary: '{done}/{total} 已完成',
      jobCreated: '测试任务已创建',
      jobCreateFailed: '创建测试任务失败',
      loadFailed: '加载失败',
      saveFailed: '保存失败',
      deleteFailed: '删除失败'
    },
