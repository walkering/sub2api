export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      batchTestFiltered: '批量测试',
      batchTestFilteredFailed: '加载批量测试账号失败',
      batchTestEmpty: '当前筛选下没有可测试账号',
      refreshCookie: '刷新 Cookie',
      testAccount: '测试账号',
      searchAccounts: '搜索账号...',
      searchPlanType: '筛选 Plan Type...',
      notes: '备注',
      openaiEmailProvider: '邮箱提供商',
      openaiEmailProviderHint: '为该 OpenAI OAuth 账号关联一个邮箱 OTP 提供商。具体连接参数在系统设置中统一配置。',
      openaiEmailProviderNone: '不关联',
      openaiEmailProviderFreemail: 'FreeMail',
      openaiPhoneProvider: '手机号提供商',
      openaiPhoneProviderHint: '为该 OpenAI OAuth 账号关联一个手机号 OTP 提供商。具体 HeroSMS 参数在系统设置中统一配置。',
      openaiPhoneProviderNone: '不关联',
      openaiPhoneProviderHeroSMS: 'HeroSMS',
      openaiStoredPassword: '登录密码',
      openaiStoredPasswordPlaceholder: '留空则使用系统默认密码（如已配置）',
      openaiStoredPasswordHint: '仅用于保存 OpenAI OAuth 账号的自动重新登录密码，不会自动回填。',
      ...
      automation: {
        title: '账号自动化',
        open: '账号自动化',
        description: '集中处理 OpenAI OAuth 账号。弹窗仅展示邮箱域名命中 FreeMail 可用域名列表的账号；仅支持已保存邮箱和密码的账号，若已配置 FreeMail 与 HeroSMS，可自动完成邮箱 OTP 和 add_phone 手机验证。',
        reload: '刷新列表',
        refresh: '批量刷新 Token',
        refreshing: '批量刷新中...',
        empty: '当前没有命中 FreeMail 可用域名列表的可自动处理账号',
        selected: '已选择 {count} 个账号',
        missingSelection: '请选择要处理的账号',
        completed: '自动化处理完成：成功 {success} 个，失败 {failed} 个，跳过 {skipped} 个',
        partial: '自动化处理部分完成：成功 {success} 个，失败 {failed} 个，跳过 {skipped} 个',
        failed: '账号自动化执行失败',
        proxyEnabled: '启用自定义代理',
        proxyEndpoint: '代理地址',
        proxyPlaceholder: '127.0.0.1:7890',
        proxyHint: '启用后，本次账号自动化任务会统一通过该代理访问 OpenAI 与邮箱 OTP 服务。',
        logs: '实时日志',
        logsEmpty: '任务启动后，这里会实时显示处理日志。',
        summary: {
          total: '总数',
          success: '成功',
          failed: '失败',
      ...
      testModel: '测试模型',
      testPromptLabel: '测试语',
      testPromptPlaceholder: '例如：hi，或请简短回复：连接测试成功。',
      testPromptHint: '可自定义发送给上游模型的测试语；留空时会使用默认测试语。',
    ...
    settings: {
      ...
      defaults: {
        ...
        openaiOAuthDefaultPassword: 'OpenAI OAuth 默认密码',
        openaiOAuthDefaultPasswordPlaceholder: '为未单独填写密码的新建 OpenAI OAuth 账号提供默认值',
        openaiOAuthDefaultPasswordHint: '仅用于新建 OpenAI OAuth 账号时保存自动重登密码，前端输入框不会自动填充。',
        openaiOAuthDefaultPasswordConfiguredPlaceholder: '已配置，留空则保持不变',
        openaiOAuthDefaultPasswordConfiguredHint: '当前已配置默认密码。留空将保持现有值，仅在新建 OpenAI OAuth 账号且未填写密码时生效。',
        openaiOAuthFreemailBaseUrl: 'OpenAI OAuth FreeMail 服务地址',
        openaiOAuthFreemailBaseUrlPlaceholder: 'https://freemail.walker-feng.workers.dev',
        openaiOAuthFreemailBaseUrlHint: '用于 OpenAI OAuth 自动重登触发邮箱 OTP 时，轮询 FreeMail 服务收件箱。',
        openaiOAuthFreemailUsername: 'OpenAI OAuth FreeMail 用户名',
        openaiOAuthFreemailUsernamePlaceholder: '请输入 FreeMail 登录用户名',
        openaiOAuthFreemailPassword: 'OpenAI OAuth FreeMail 密码',
        openaiOAuthFreemailPasswordPlaceholder: '请输入 FreeMail 登录密码',
        openaiOAuthFreemailPasswordHint: '仅用于系统自动轮询邮箱 OTP，不会自动回填到其他表单。',
        openaiOAuthFreemailPasswordConfiguredPlaceholder: '已配置，留空则保持不变',
        openaiOAuthFreemailPasswordConfiguredHint: '当前已配置 FreeMail 密码。留空将保持现有值，仅用于自动补全邮箱 OTP。',
        openaiOAuthFreemailDomain: 'OpenAI OAuth FreeMail 可用域名列表',
        openaiOAuthFreemailDomainPlaceholder: '例如：webcode.team,mail.example.com',
        openaiOAuthFreemailDomainHint: '使用逗号分隔多个可用域名，第一个域名会作为默认域名。账号自动化仅展示并处理邮箱域名在该列表中的账号。',
        openaiOAuthEmailOTPAdvanced: 'OpenAI OAuth 邮箱 OTP 高级设置',
        openaiOAuthEmailOTPPollIntervalMsPlaceholder: '邮箱 OTP 轮询间隔（毫秒）',
        openaiOAuthEmailOTPResendAfterSecondsPlaceholder: '邮箱 OTP 超时重发秒数',
        openaiOAuthEmailOTPPollAttemptsPlaceholder: '邮箱 OTP 单轮轮询次数',
        openaiOAuthEmailOTPAdvancedHint: '控制 OpenAI OAuth 邮箱验证码轮询频率、超时重发等待时间和每轮轮询次数。',
        openaiOAuthPhoneBaseUrl: 'OpenAI OAuth HeroSMS 接口地址',
        openaiOAuthPhoneBaseUrlPlaceholder: 'https://hero-sms.com/stubs/handler_api.php',
        openaiOAuthPhoneBaseUrlHint: '用于 OpenAI OAuth 自动重登命中 add_phone 时，向 HeroSMS 申请号码并轮询短信。',
        openaiOAuthPhoneApiKey: 'OpenAI OAuth HeroSMS API Key',
        openaiOAuthPhoneApiKeyPlaceholder: '请输入 HeroSMS API Key',
        openaiOAuthPhoneApiKeyHint: '仅用于系统自动接收手机号验证码，不会自动回填到其他表单。',
        openaiOAuthPhoneApiKeyConfiguredPlaceholder: '已配置，留空则保持不变',
        openaiOAuthPhoneApiKeyConfiguredHint: '当前已配置 HeroSMS API Key。留空将保持现有值，仅用于自动补全手机号 OTP。',
        openaiOAuthPhoneServiceCode: 'OpenAI OAuth HeroSMS 服务代码',
        openaiOAuthPhoneServiceCodePlaceholder: '例如：OpenAI',
        openaiOAuthPhoneCountry: 'OpenAI OAuth HeroSMS 国家 ID',
        openaiOAuthPhoneCountryPlaceholder: '例如：52',
        openaiOAuthPhoneOperator: 'OpenAI OAuth HeroSMS 运营商',
        openaiOAuthPhoneOperatorPlaceholder: '可选，多个运营商用英文逗号分隔',
        openaiOAuthPhoneMaxPrice: 'OpenAI OAuth HeroSMS 最高价格',
        openaiOAuthPhoneMaxPricePlaceholder: '0 表示不限制',
        openaiOAuthPhoneAdvanced: 'OpenAI OAuth 手机验证码高级参数',
        openaiOAuthPhoneFixedPrice: '固定价格购买',
        openaiOAuthPhoneLeaseMinutesPlaceholder: '租约分钟数，默认 20',
        openaiOAuthPhonePollIntervalMsPlaceholder: '短信轮询间隔（毫秒），默认 5000',
        openaiOAuthPhoneResendAfterSecondsPlaceholder: '等待多少秒后触发重发，默认 30',
        openaiOAuthPhoneAdvancedHint: '这些参数会用于 HeroSMS 取号、短信轮询和验证码重发策略。',
